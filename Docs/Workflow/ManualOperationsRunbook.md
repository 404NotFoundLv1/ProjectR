...
**触发条件**：Codex 已报告 Build、原生自动化、Blueprint 编译、精确保存、Dirty=0、重启回载和自动 PIE 输入全部 PASS，并在 `L_CombatGym` 留下已停止的 PIE。

1. 在 Selected Viewport 启动单玩家 PIE。
2. 分别按住 A、D，确认实际移动方向立即改变，但 Mesh 在约 0.12 秒内平滑转向，松开后保持最后朝向。
3. 朝一个方向跳起后，在空中按相反方向，确认角色立即向相反方向产生实际位移，而不只是 Mesh 转身；同时确认没有可感知的 Y/前后漂移。
4. 快速交替 A/D 约 10 次，并连续跳跃至少 5 次，至少重复三次空中反向；确认每次视觉转向从当前角度继续，不瞬移到端点、不来回振荡。
5. 确认固定侧视相机没有换边、自由旋转、抖动或明显裁切。
6. 判断当前默认速度、跳高和构图是否达到“可继续开发”；本版本不在此步骤调数值。
7. StopPIE，返回 `平滑转向手感：PASS` 或 `FAIL`。不要测试或要求 Attack/Dodge/Interact/Execute 产生玩法效果。

**实体手柄**：资产映射与自动注入已经客观验证；实体手柄手感记录 `NOT RUN`，不阻断当前版本。

**失败处理**：返回失败动作、复现次数和观察结果；不要修改资产、Config、Blueprint 或保存 Package，由 Codex继续修复当前版本并重跑自动门。
