...
- `UPRDirectorSubsystem` 仍是唯一的法令 Runtime owner。它在不修改 `FPRDirectorRequest`、`FPRDirectorResponse`、Validator、Registry 或 `FPRAppliedDirectorRuleHandle` 的前提下，公开只读 `FPRDirectorRuleRuntimeState`（HandleId、RuleId、Level、Status、原因/效果/反制文本、进度、降级标记、序列与时间）及查询/变化事件。规则执行、反制进度、GE、Timer 和绑定不进入 Widget、Save 或下游原始 Provider 数据。
- Registry 现在固定十二项 RuleId：`Rule.CompanionIsolation`、`Rule.CooperationAudit`、`Rule.DeleteEcho`、`Rule.DistanceCorrection`、`Rule.EmotionalInterference`、`Rule.ObedienceTest`、`Rule.OptimalPath`、`Rule.PredictionLock`、`Rule.RepetitionPenalty`、`Rule.ResourceBalance`、`Rule.RiskReward`、`Rule.SurvivalProtocol`。未知 Rule、非法参数和 Development Debug 请求都必须走同一 Validator，不能创建 Handle 或执行效果。
- `FPRDirectorRuleEffectExecutor` 是唯一会话内效果 owner。它只用六个固定 GAS GE 及窄 Companion SourceId 支援策略/Enemy SpawnId 只读 seam，按确定性乘积与 Clamp 聚合；Replace、Remove、Profile session reset、World cleanup、PIE Stop 与 Deinitialize 必须准确移除本 Rule 创建的 GE、Timer、Delegate、Enemy modifier、Companion modifier 和计数。Combat、Ability、QTE 继续只发布既有值型事件，绝不 include 或反向调用 Director。
- `UPRDirectorRulePanelWidget` 只读取 RuntimeState，并显示原因、效果、反制及进度；没有激活 Rule 时折叠。它不能创建伤害、GameplayEffect、奖励、保存、生成或状态变化。普通旅行只做幂等重新绑定；成功 Create/Load Profile 才清除法令会话。
- v0.4.1 的 DeleteEcho、OptimalPath、ResourceBalance、RiskReward 与 ObedienceTest 都是明确标记的可逆会话内降级语义；它们不伪造 Room、Reward、经济、Account、Graveyard 或持久化。v0.4.2 只能读取已验证 Handle/RuntimeState，不能读取原始 Provider Response。
...
# v0.6.0 Allocator chapter and Human Anomaly Proof contract

**Owner:** `UPRChapterSubsystem` (`UGameInstanceSubsystem`). **Established:** v0.6.0. **Direct consumers:** the Allocator overlay, later closed chapter registries, public proof readers, Steam Cloud and QA.

- The subsystem is the only owner of the closed Allocator Chapter/Content Registry, frozen RunId/AccountId/Seed facts, local directive, `AllocationPressure`, proof settlement/retry, value snapshots and world lifecycle. Its public surface is limited to `GetSnapshot`, `GetLatestCompletion`, `RetryPendingSettlement`, `OnStateChanged`, and `OnChapterCompleted`; it accepts no Blueprint write request or arbitrary chapter/profile/seed/rule/enemy/class/path input.
- The fixed content identities are `ProjectRChapter:DA_Chapter_Allocator`, `Allocator`, `Allocator`, and `HumanAnomalyProof.Allocator`. The derived Room Registry closes exactly 10 Rooms, 4 Encounters, 4 Events, 5 RewardPolicies, the existing 30 Rewards, 5 local directives and 10 pressure bindings. The Enemy Registry closes exactly the Allocator Boss and three Allocator variants by sorted PrimaryAssetId. Unknown, duplicate, unsorted or unclosed content is rejected before travel, spawn or reward application.
- Chapter local directives remain `FName` values inside the chapter Registry only. They never enter Director Request/Response, Validator, Registry, RuntimeState or executor. The four event results change only transient `AllocationPressure`, clamped to `0..4`; zero relationship deltas are required and the result is never persisted. Shop remains the existing three-choice, non-reroll Reward Offer and has no currency, price, stock, transaction or global-economy meaning.
- `APRAllocatorBoss` and `UPRAllocatorBossComponent` publish one value completion only. Resource Lock, Reward Deprivation and Price Audit are phase-bounded, readable, counterable, degraded-safe and cleaned on phase completion, travel, EndPlay, PIE stop and deinitialization. They persist neither Actor, ASC/GE Handle, Timer, Delegate nor reward handle.
- Schema 7 appends only `FPRChapterPersistenceData`: at most five sorted unique `CompletedChapterIds`, at most five sorted unique `HumanAnomalyProofIds`, and `SettlementSequence`. Migration remains strictly `1->2->3->4->5->6->7`; 6->7 is empty and never retroactively awards a proof. PRSV A/B selection, corruption recovery, future-version refusal, write-back verification and frozen retry semantics remain owned by Save.
- `HumanAnomalyProof.Allocator` is a non-consumable Profile chapter qualification, separate from CounterproofFragments and MemoryFragments. It is eligible only after matching frozen RunId/AccountId/Seed, a closed Allocator RoomSequence ending at the Allocator Boss, the one-time Allocator Boss completion, and a successful `RoomSequenceCompleted` AccountDeleted fact. Publication occurs only after Schema-7 write-back; duplicates, failed writes, non-completed termination, invalid paths and prior proof ownership are no-ops.

# v0.6.1 Warden additive chapter contract

**Owner:** `UPRChapterSubsystem` (`UGameInstanceSubsystem`). **Established:** v0.6.1. **Direct consumers:** closed Chapter content, Warden overlay, public proof readers and later chapter registries.

- The subsystem extends the chapter whitelist only with `ProjectRChapter:DA_Chapter_Warden`, content/boss `Warden`, and `HumanAnomalyProof.Warden`. A missing Allocator proof selects Allocator; an Allocator proof selects Warden; an existing Warden proof permits replay but makes settlement a no-op. It owns only frozen Chapter value facts, local RiskPressure/directive state, proof retry, snapshots and lifecycle; it accepts no caller-provided chapter, registry, class, path, seed or spawn input.
- The Warden Room Registry is a sorted, closed PrimaryAssetId set of exactly 10 Rooms, 4 Encounters, 4 Events, 5 Reward Policies and 5 local directives. The Warden Enemy Registry is the sorted closed set of the Warden Boss and three Warden variants. Before Warden encounter resolution, `UPRChapterSubsystem` dynamically registers only the compiled-in exact Warden Enemy Registry path with exact Asset Manager bundle metadata and confirms its typed ID/path. It never scans asset paths or changes Config, Asset Manager settings or `UPREnemySubsystem`.
- Seeds `61100..61104` deterministically select a 6..10 Room path and one fixed local directive. Events alter only transient `RiskPressure`, clamped `0..4`; the RiskProvisionTerminal is the existing non-reroll three-choice Reward Offer and has no currency, price, stock or transaction semantics. Local Warden directives consume only known validated Director Handle facts and never enter Director Request/Response, Validator, Registry, RuntimeState or executor.
- `APRWardenBoss` and `UPRWardenBossComponent` own Predictive Attack, three-lane warning/Lockdown and Risk Mark phases. Each has a readable counter, degraded-safe no-op behavior, exactly one completion, and idempotent cleanup on completion, travel, EndPlay, PIE stop and deinitialization. No Actor, Timer, Delegate, Handle, Widget or runtime target is persisted. The fixed Axiom StoryBeat reads public Companion/Quest/Memory snapshots only and writes no relationship, quest, memory, save or dialogue state.
- Schema 7 is unchanged. `HumanAnomalyProof.Warden` is appended idempotently to the existing bounded Chapter/Proof sets only after matching frozen RunId/AccountId/Seed, closed Warden Room sequence, Warden Boss completion and successful existing account-termination fact. A/B write-back verification precedes publication; retry reuses the frozen transaction. Duplicate callbacks, prior proof, invalid sequence and failed writes never publish or compensate a proof.

# v0.6.2 Pacifier additive chapter contract

**Owner:** `UPRChapterSubsystem` (`UGameInstanceSubsystem`). **Established:** v0.6.2. **Direct consumers:** closed Chapter content, the Pacifier overlay, public proof readers, v0.7.0 and QA.

- The fixed chapter closure is exactly Allocator, Warden and Pacifier. Selection consumes only the public Schema-7 chapter snapshot: no Allocator proof selects Allocator; Allocator without Warden selects Warden; Allocator plus Warden selects Pacifier; an existing Pacifier proof permits replay without a second settlement. A Warden proof without Allocator, or a Pacifier proof without both prerequisites, is rejected as `Chapter.InvalidProofChain`.
- Pacifier identities and Registry paths are compiled-in constants. Its Room Registry closes exactly 10 Rooms, 4 Encounters, 4 Events, 5 Reward Policies, the existing 30 Rewards, 5 local directives and 10 pressure bindings. Its Enemy Registry closes exactly the Pacifier Boss and three Pacifier variants. Dynamic Asset Manager registration accepts only the exact compiled-in Pacifier Registry ID/path and bundle metadata; it performs no scan and accepts no caller-controlled asset, class, path, tag or spawn input.
- Seeds `61200..61204` deterministically select 6..10 Rooms and one fixed local directive. Events alter only transient `ComfortPressure`, clamped to `0..4`. Constraint exhaustion is a visible deterministic no-op. `SafetyProvisionTerminal` remains the existing non-reroll three-choice Reward Offer; SafeRewardOffer and Adventure Yield Suppression have no currency, price, stock, trade, drop-rate, RewardData or persistent-economy meaning.
- `APRPacifierBoss` and `UPRPacifierBossComponent` own Illusion Split, Low-risk Reward Lure and Adventure Yield Suppression. Illusion projections are transient, non-placeable Actors with no collision, ASC, Combatant, target, completion or Save identity. All player damage is submitted through `UPRCombatSubsystem`; each phase and counter is idempotent, degraded dependencies are a visible no-op, completion publishes once, and projections/timers/delegates/references are cleared on phase end, death, travel, EndPlay, PIE stop and deinitialization.
- The fixed Kindle StoryBeat reads only public Companion relationship/Primary, completed Quest entitlement and bounded Memory summary snapshots. It publishes no Relationship, Quest, Memory, Progression, Save or Dialogue mutation and never reads Evidence, raw graveyard data, private Memory, Dialogue queue, Prompt or Provider state.
- Schema 7 and Save Runtime/Migration are unchanged. `HumanAnomalyProof.Pacifier` is appended idempotently to the existing maximum-five Chapter/Proof sets only after both prerequisite proofs, matching frozen RunId/AccountId/Seed, the closed Pacifier Room sequence, matching one-time Boss completion, successful `RoomSequenceCompleted` account termination and A/B write-back verification. Retry reuses the frozen transaction; invalid chains, duplicates, failed writes, death, evacuation and interruption never publish or compensate a proof.
- Compatibility correction: the global `FPRCombatEvent` stream contains both player-target and enemy-target facts. `UPRRunStateSubsystem` may aggregate damage and begin account-death termination only for the current player target. A valid transient Target actor must equal the current player Pawn; actor-less bounded legacy facts are accepted only when the stable `TargetId` is exactly `Player`. This does not change Account/Run lifecycle, Save schema, Combat publication or public API, and prevents enemy death during a chapter Encounter from being misclassified as player death.
...
# v0.7.2 Triple Resonance contract

**Owner:** `UPRTripleResonanceSubsystem` (`UGameInstanceSubsystem`). **Direct consumers:** Headmind opportunity, `UPRQTESubsystem`, isolated Schema-8 persistence, Hub presentation, v0.8.0 plugin-pool consumers, QA and release.

- The subsystem freezes exactly one public `FPRTripleResonanceOpportunitySnapshot` identity: active Headmind World, RunId, AccountId, Boss SpawnId and opportunity window. It accepts only `EligibleDeferredToV072`, the derived Auditor prerequisite, the StartRun-frozen `ProgressionNode:BondTripleResonance` entitlement, Trust >= 70 and Overload == 0 for Axiom/Kindle/Null. Unknown, stale, duplicate, closed-window or mismatched identities are no-ops.
- Its state/value contract is `Unavailable -> Ready -> QTEActive -> AbilityPending -> Executing -> Resolved`, with `Failed` and `ReadyToRetry` exits. Snapshot/result/legacy/retry APIs and native delegates expose bounded values only; there is no Blueprint writer, arbitrary ID/path/provider input or runtime Registry setter.
- The external ResultOnly QTEs are exactly Axiom/Kindle/Null: Interact, Attack, Execute in that order, each 1.25 seconds and RelationshipDelta zero. They never enter the twelve P0 Registry/candidate ordering and QTE owns request, timer, input and cleanup.
- The transient ability charges 100 Resonance, commits one 100-cost/90-second-cooldown execution, submits one 120-base-damage `Skill.TripleResonance` Combat request, and counters the Headmind Basilisk through its narrow public seam. It neither changes Demo Auditor P1--P3, PredictionShield, mitigation, BossSubsystem nor creates another completion.
- Schema 8 appends only one bounded P0 skill-memory fragment, one independent high-risk proof, waterline and sequences. A matching AccountDeleted fact freezes and atomically write-back-verifies a high-risk transaction; failure publishes no legacy/proof and retries the same transaction. Account ownership still terminates ActiveAccount.
- High-risk eligibility is matching RoomSequenceCompleted account termination, Headmind completion/counter, SynthesisPressure 4, minimum health <= 25%, a new GraveyardOrdinal and a committed canonical P0 skill. Deterministic selection is CommitCount, UseCount, AbilityTag. Replay, waterline, invalid identity, death, evacuation and interruption never compensate or increment. Next StartRun exposes a read-only Legacy snapshot only.
- Widgets are presentation-only fixed local text: eligibility, inputs, timer, result, saved legacy/proof and readable failure states without color-only cues. QTE, transient ability/GE, Widgets, timers, delegates and weak references clear on all lifecycle exits.

**Status:** Accepted and implemented (v0.7.2).
