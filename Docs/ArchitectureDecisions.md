...
# ADR-038 - Auditor is an additive fourth closed Chapter projection

**Status:** Accepted and implemented (v0.7.0 implementation evidence complete; version tracking remains In Progress pending its separate release action).

**Context:** The frozen Chapter closure ended at Pacifier, while the frozen Demo Auditor owns P1--P3, BossGym, mitigation, PredictionShield and one prototype completion. Chapter four needs a deterministic Auditor path, bounded audit mechanisms and a fourth Human Anomaly Proof without changing those owners, Schema 7, Save runtime, Director, Combat, Enemy base, RealityHub, map or GameplayTags.

**Decision:** `UPRChapterSubsystem` accepts only the additive Allocator/Warden/Pacifier/Auditor whitelist and selects Auditor only from the complete public prior-proof chain. Its compiled-in Auditor registries close content to exact PrimaryAssetIds and fixed paths. `APRAuditorChapterBoss : APRBossAuditor` plus its chapter component consume only public prototype state to add transient Habit, Build and Verdict windows; the existing Demo component remains unmodified. The Room subsystem defers encounter completion by one tick after a Boss health-delegate completion so Combat can first publish `State.Dead`; this removes the observed fatal-damage ordering race without changing BossSubsystem, Combat or the Demo Auditor. The conditional fixed no-argument PIE fixture is registered as `PRAuditorAutomationToolset` (the exact available implementation name for the planned Auditor automation toolset); it has no caller-controlled content or storage input. `AuditPressure`, local directives, Null StoryBeat and TripleResonance prerequisite are value-only chapter projections; no economy, Director Rule, second completion, second RunSummary or new persistent schema is created.

**Consequences:** Schema remains strictly `1->2->3->4->5->6->7`. `HumanAnomalyProof.Auditor` occupies the fourth existing bounded Chapter/Proof slot only after the frozen AccountDeleted fact and A/B write-back verification; failure retries the same frozen transaction and replay cannot compensate or increment settlement. The derived TripleResonance prerequisite is read-only for v0.7.2. Runtime windows, Widgets, Actors, Timers, Delegates and weak references are not persisted and are cleared on all lifecycle exits.

**Migration/rollback:** No migration is introduced. Rollback is limited to v0.7.0 Auditor C++/UI, tests, narrow Chapter/Room additions, conditional fixed editor-only Toolsets, exact 37 new Packages, exact ordinary-Git attributes and v0.7.0 evidence documents. A saved Package is never deleted without separate per-package approval.
