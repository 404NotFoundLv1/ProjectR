...
| E-037 | v6 只给出 Pacifier 的幻象、安全奖励、情绪干扰、三种 Boss 机制、三种敌人和燧火剧情方向；真实 v0.6.1 仍含 Warden 专用选择标记、Registry 注册、RiskPressure、Room/Boss 分支和展示逻辑。冻结 Reward/Director/Save/Combat 合同不允许把“安全奖励”解释为经济、把“收益削弱”实现为 RewardData/掉率修改，或把幻象实现为正式 Enemy/地图 Actor。用户要求 v0.6.2 一次完整交付，不拆分子版本、检查点或中间功能提交 | v0.6.2 仅把私有章节闭包加法扩展为固定 Allocator/Warden/Pacifier 三项，并按 `Allocator -> Warden -> Pacifier` Proof 链自动选择；不增加章节选择 UI、路径扫描、任意 Registry 输入或通用加载器。Pacifier 使用独立 `ComfortPressure[0..4]`，不复用 AllocationPressure/RiskPressure。SafeRewardOffer 与 SafetyProvisionTerminal 只消费既有三选一不可重掷 Reward Offer；Adventure Yield Suppression 只形成章节局部路线/战斗压力，不改 RewardData、Reward Handle、掉率或经济。幻象是 Pacifier Boss 所有的无碰撞、无 ASC/Combat/Completion 身份的瞬态原生投影，结束时完整清理。固定 Pacifier Enemy Registry 复用 Warden 的 bundle-backed 精确动态注册边界，但只接受编译期固定 ID/路径。Schema 保持 7，仅在完整前置 Proof 链和写后回读成功后幂等追加 Pacifier | 满足第三章情绪稳定主题和端到端通关，同时保持 Allocator/Warden、Director、Reward、Combat、地图、Save A/B 与未来 v0.7.0 扩展边界 | Active for v0.6.2 |

# v0.7.0 启动合同勘误

| 编号 | 路线/接口缺口 | v0.7.0 决策 | 原因 | 当前状态 |
|---|---|---|---|---|
